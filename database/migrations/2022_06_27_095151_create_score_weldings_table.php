...
class CreateScoreWeldingsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('score_weldings', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('quali_id');
            $table->unsignedBigInteger('welding_crit_id');
            $table->unsignedBigInteger('welding_quide_id');
            ...
            $table->foreign('welding_quide_id')
            ->references('id')
            ->on('guideline_weldings')
            ->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('score_weldings');
    }
}
