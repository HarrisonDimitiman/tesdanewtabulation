...
class CreateScoreAsexualsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('score_asexuals', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('quali_id');
            $table->unsignedBigInteger('asexual_crit_id');
            $table->unsignedBigInteger('asexual_quide_id');
            ...
            $table->foreign('asexual_quide_id')
            ->references('id')
            ->on('guideline_asexuals')
            ->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('score_asexuals');
    }
}
